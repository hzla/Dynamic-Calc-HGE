...
function calculateSMSS(gen, attacker, defender, move, field) {
    ...
    var baseDamage = calculateBaseDamageSMSS(gen, attacker, defender, basePower, attack, defense, move, field, desc, isCritical);
    ...
    if ((move.dropsStats && move.timesUsed > 1) || move.hits > 1) {
        ...
        var _loop_1 = function (times) {
            ...
            var newAttack = calculateAttackSMSS(gen, attacker, defender, move, field, desc, isCritical);
            var newDefense = calculateDefenseSMSS(gen, attacker, defender, move, field, desc, isCritical);
            ...
            var newBasePower = calculateBasePowerSMSS(gen, attacker, defender, move, field, hasAteAbilityTypeChange, desc, times + 1);
            var newBaseDamage = calculateBaseDamageSMSS(gen, attacker, defender, newBasePower, newAttack, newDefense, move, field, desc, isCritical);
            var newFinalMods = calculateFinalModsSMSS(gen, attacker, defender, move, field, desc, isCritical, typeEffectiveness, times);
...
function calculateBasePowerSMSS(gen, attacker, defender, move, field, hasAteAbilityTypeChange, desc, hit) {
    var _a;
    if (hit === void 0) { hit = 1; }
    var turnOrder = attacker.stats.spe > defender.stats.spe ? 'first' : 'last';
    var basePower;
...
function calculateBPModsSMSS(gen, attacker, defender, move, field, desc, basePower, hasAteAbilityTypeChange, turnOrder) {
    ...
    if ((attacker.hasAbility('Reckless') && (move.recoil || move.hasCrashDamage)) ||
        ...
        (attacker.hasAbility('Liquid Voice') && !!move.flags.sound && TITLE == "Aurora Crystal")) {
        if (attacker.hasAbility('Iron Fist') && TITLE == "Aurora Crystal") {
            bpMods.push(5325);
        } else {
            bpMods.push(4915);
        }
    ...
    else if (TITLE == "Aurora Crystal" && (attacker.curHP() > attacker.maxHP() / 3 &&
    ((attacker.hasAbility('Overgrow') && move.hasType('Grass')) ||
        (attacker.hasAbility('Blaze') && move.hasType('Fire')) ||
        (attacker.hasAbility('Torrent') && move.hasType('Water')) ||
        (attacker.hasAbility('Swarm') && move.hasType('Bug'))) || (attacker.hasAbility('Hyper Cutter')))) {
        bpMods.push(4505);
        desc.attackerAbility = attacker.ability;
    }
...
function calculateAtModsSMSS(gen, attacker, defender, move, field, desc) {
    ...
    if ((attacker.hasAbility('Slow Start') && attacker.abilityOn &&
        ...
        (attacker.curHP() <= attacker.maxHP() / 4 && attacker.hasAbility('Swan Song')) ||
        (move.category === 'Special' && attacker.abilityOn && attacker.hasAbility('Plus', 'Minus')) ||
        (attacker.hasAbility('Aqua Adept') && move.hasType('Water')) ||
        (attacker.hasAbility('Flower Power') && move.hasType('Grass'))|| 
        (attacker.hasAbility('Lunar Energy') && move.hasType('Fairy')) ||
        (attacker.hasAbility('Odd Power') && move.hasType('Psychic')) ||
        (attacker.hasAbility('Solar Energy') && move.hasType('Fire')) ||
        (attacker.hasAbility('Wind Whipper') && move.hasType('Flying'))) {
        atMods.push(6144);
        desc.attackerAbility = attacker.ability;
    }
        ...
        atMods.push((gen.num >= 9 && TITLE != "Aurora Crystal") ? 5325 : 6144);
        desc.attackerAbility = attacker.ability;
    }
    ...
    else if ((attacker.hasAbility('Conductor') && move.flags.sound) || (attacker.hasAbility('Water Bubble') && move.hasType('Water')) ||
...
function calculateDfModsSMSS(gen, attacker, defender, move, field, desc, isCritical, hitsPhysical) {
    ...
    else if (TITLE == "Aurora Crystal" && defender.hasAbility('Big Pecks')) {
        dfMods.push(4506);
        desc.defenderAbility = defender.ability;
    }
...
function calculateFinalModsSMSS(gen, attacker, defender, move, field, desc, isCritical, typeEffectiveness, hitCount) {
    if (isCritical === void 0) { isCritical = false; }
    ...
    else if ((defender.hasAbility('Punk Rock') || (defender.hasAbility('Cacophony')) && move.flags.sound) ||
